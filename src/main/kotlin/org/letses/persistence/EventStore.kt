...
interface EventStore<E : Event> {

    suspend fun read(stream: String, from: EventVersion): List<PersistentEventEnvelope<E>> {
        val results = LinkedList<PersistentEventEnvelope<E>>()
        read(stream, from) { results.add(it) }
        return results
    }
